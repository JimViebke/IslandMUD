...
std::string PC::generate_area_map(const World & world, const std::map<std::string, std::shared_ptr<Character>> & actors) const
{
	...
	for (int cx = x - (int)C::VIEW_DISTANCE; cx <= x + (int)C::VIEW_DISTANCE; ++cx)
	{
		...
		for (int cy = y - (int)C::VIEW_DISTANCE; cy <= y + (int)C::VIEW_DISTANCE; ++cy)
		{
			...
			{
				forest_grid[i][cy - (y - (C::VIEW_DISTANCE + 1))] = world.room_at(cx, cy)->contains(C::TREE_ID);
			}
		}
	}
	...
	for (int cx = x - (int)C::VIEW_DISTANCE; cx <= x + (int)C::VIEW_DISTANCE; ++cx)
	{
		...
		for (int cy = y - (int)C::VIEW_DISTANCE; cy <= y + (int)C::VIEW_DISTANCE; ++cy)
		{
			...
			if (world.room_at(cx, cy) == nullptr)
			{
				// draw the "room"
				a.push_back(C::LAND_CHAR); a.push_back(C::LAND_CHAR); a.push_back(C::LAND_CHAR);
			...
			else if (world.room_at(cx, cy)->is_water())
			{
			...
			else
			{
				...
				const bool n = world.room_at(cx, cy)->has_surface(C::NORTH);
				const bool e = world.room_at(cx, cy)->has_surface(C::EAST);
				const bool s = world.room_at(cx, cy)->has_surface(C::SOUTH);
				const bool w = world.room_at(cx, cy)->has_surface(C::WEST);
				...
				*/
				if (n)
				{
					nd = world.room_at(cx, cy)->get_room_sides().find(C::NORTH)->second.has_door();
					nr = (!world.room_at(cx, cy)->is_standing_wall(C::NORTH) ||
						(nd && world.room_at(cx, cy)->get_room_sides().find(C::NORTH)->second.get_door()->is_rubble()));
				}
				if (e)
				{
					ed = world.room_at(cx, cy)->get_room_sides().find(C::EAST)->second.has_door();
					er = (!world.room_at(cx, cy)->is_standing_wall(C::EAST) ||
						(ed && world.room_at(cx, cy)->get_room_sides().find(C::EAST)->second.get_door()->is_rubble()));
				}
				if (s)
				{
					sd = world.room_at(cx, cy)->get_room_sides().find(C::SOUTH)->second.has_door();
					sr = (!world.room_at(cx, cy)->is_standing_wall(C::SOUTH) ||
						(sd && world.room_at(cx, cy)->get_room_sides().find(C::SOUTH)->second.get_door()->is_rubble()));
				}
				if (w)
				{
					wd = world.room_at(cx, cy)->get_room_sides().find(C::WEST)->second.has_door();
					wr = (!world.room_at(cx, cy)->is_standing_wall(C::WEST) ||
						(wd && world.room_at(cx, cy)->get_room_sides().find(C::WEST)->second.get_door()->is_rubble()));
				}
				...
				char nw_corner = C::LAND_CHAR, ne_corner = C::LAND_CHAR, se_corner = C::LAND_CHAR, sw_corner = C::LAND_CHAR;
				{
					...
					const bool wtn = world.room_has_surface(cx - 1, cy, C::WEST);
					const bool wte = world.room_has_surface(cx, cy, C::NORTH);
					const bool wts = world.room_has_surface(cx, cy, C::WEST);
					const bool wtw = world.room_has_surface(cx, cy - 1, C::NORTH);
				...
				}
				{
					...
					const bool wtn = world.room_has_surface(cx - 1, cy, C::EAST);
					const bool wte = world.room_has_surface(cx, cy + 1, C::NORTH);
					const bool wts = world.room_has_surface(cx, cy, C::EAST);
					const bool wtw = world.room_has_surface(cx, cy, C::NORTH);
				...
				}
				{
					...
					const bool wtn = world.room_has_surface(cx, cy, C::EAST);
					const bool wte = world.room_has_surface(cx, cy + 1, C::SOUTH);
					const bool wts = world.room_has_surface(cx + 1, cy, C::EAST);
					const bool wtw = world.room_has_surface(cx, cy, C::SOUTH);
				...
				}
				{
					...
					const bool wtn = world.room_has_surface(cx, cy, C::WEST);
					const bool wte = world.room_has_surface(cx, cy, C::SOUTH);
					const bool wts = world.room_has_surface(cx + 1, cy, C::WEST);
					const bool wtw = world.room_has_surface(cx, cy - 1, C::SOUTH);
				...
				b.push_back(((cx == x && cy == y) ? C::PLAYER_CHAR
					...
					: ((world.room_at(cx, cy)->has_table()) ? C::TABLE_CHAR
					// else if there is a chest, draw a chest
					: ((world.room_at(cx, cy)->has_chest()) ? C::CHEST_CHAR
					// else if there is a non-mineral deposit item, draw an item char
					: ((world.room_at(cx, cy)->has_non_mineral_deposit_item()) ? C::ITEM_CHAR
					// else if there is a mineral deposit, draw a mineral char
					: ((world.room_at(cx, cy)->has_mineral_deposit()) ? C::GENERIC_MINERAL_CHAR
