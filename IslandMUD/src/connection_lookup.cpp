...
void connection_lookup::set_connection(const character_id & user_ID, const network::connection_ptr & connection)
{
	connections.get(user_ID, connections.get_write_lock()).user_connection = connection;
}
// save the user's map connection
void connection_lookup::set_map_connection(const character_id & user_ID, const network::connection_ptr & map_connection)
{
	connections.get(user_ID, connections.get_write_lock()).map_connection = map_connection;
}

// retrieve a user's connection
network::connection_ptr connection_lookup::get_connection(const character_id & user_ID) const
{
	return connections.get(user_ID, connections.get_read_lock()).user_connection;
}
// retrieve a user's map connection
network::connection_ptr connection_lookup::get_map_connection(const character_id & user_ID) const
{
...
character_id connection_lookup::get_user_ID(const network::connection_ptr & connection) const
{
...
void connection_lookup::erase(const character_id & user_ID)
{
